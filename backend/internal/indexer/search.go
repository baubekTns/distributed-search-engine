...
func (c *Client) Search(ctx context.Context, options SearchOptions) (SearchResponse, error) {
    ...
    if options.Query == "" { return SearchResponse{}, errors.New("search query cannot be empty") }
    ...
    if options.CrawledAfter != nil { dateRange["gte"] = options.CrawledAfter.UTC().Format(time.RFC3339) }
    if options.CrawledBefore != nil { dateRange["lte"] = options.CrawledBefore.UTC().Format(time.RFC3339) }
    if len(dateRange) > 0 { filters = append(filters, map[string]any{"range": map[string]any{"crawled_at": dateRange}}) }
    ...
    if len(filters) > 0 { boolQuery["filter"] = filters }
    ...
    for _, hit := range raw.Hits.Hits {
        ...
        if highlighted := firstHighlight(hit.Highlight, "title"); highlighted != "" { title = highlighted }
        snippet := firstHighlight(hit.Highlight, "content")
        if snippet == "" { snippet = truncateText(hit.Source.Content, 180) }
